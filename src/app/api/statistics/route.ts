...
export async function GET(request: NextRequest) {
  try {
    ...
    const paidMinutes = timeTracks
      .filter(track => track.was_paid)
      .reduce((sum, track) => sum + track.time, 0)
    
    const unpaidMinutes = timeTracks
      .filter(track => !track.was_paid)
      .reduce((sum, track) => sum + track.time, 0)
    
    // Подсчитываем уникальные дни с треками
    const uniqueDays = new Set(timeTracks.map(track => 
      track.date.toISOString().split('T')[0]
    )).size
